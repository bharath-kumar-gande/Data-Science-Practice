...
import pandas as pd
import os
from sklearn import tree, model_selection, ensemble,feature_selection
...
# =============================================================================
#rand_estimator = ensemble.RandomForestClassifier(n_estimators=9, criterion= 'entropy', max_depth= 3, max_features=15, min_samples_split= 2)
rand_estimator = ensemble.RandomForestClassifier(n_estimators=9)
rand_estimator.fit(X_train,y_train)
fs = feature_selection.SelectFromModel(rand_estimator, threshold='median',prefit=True)
X_train1 = fs.transform(X_train)


rand_estimator_grid = ensemble.RandomForestClassifier(random_state=10)
param_grid = {'n_estimators':list(range(5,10)),'max_features':list(range(5,11)), 'max_depth':list(range(1,6)), 'min_samples_split':list(range(2,8)),'criterion':['gini','entropy']}
rand_grid = model_selection.GridSearchCV(rand_estimator_grid, param_grid, cv=10, n_jobs=5)
rand_grid.fit(X_train1,y_train)
...
print(rand_grid.score(X_train1, y_train)) #Train score  #Evalution of tree
...
g_test['type1'] = rand_grid.predict(X_test1)
g_test.info()
